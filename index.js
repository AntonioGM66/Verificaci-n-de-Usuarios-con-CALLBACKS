...
function verificarUsuario(usuario, callback) { //parametros
    setTimeout(() => {
    // Simulamos que los usuarios 'ana' y 'luis' existen
    const usuariosExistentes = ['ana', 'luis'];
    const existe = usuariosExistentes.includes(usuario); //.includes metodo que nos permite encontrar un elemento en un array
    callback(existe);
    }, 1000);
}
